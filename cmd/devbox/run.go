...
func runAction(cmd *cobra.Command, args []string) error {
	...
	opts = append(opts, func(_ context.Context, _ oci.Client, _ *containers.Container, spec *oci.Spec) error {
		//spec.Process.Args = append([]string{"ash"}, spec.Process.Args...)
		logrus.Debugf("Mounting the files that we require!")
		spec.Mounts = append([]specs.Mount{{
			Destination: "/tmp/file",
			Type:        "bind",
			Source:      "/tmp/currentwall",
			Options:     []string{"bind", "rw"}, // ro is also valid. is rw the best choice here?
		}}, spec.Mounts...)
		return nil
	})
